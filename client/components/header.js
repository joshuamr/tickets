...
const Header = ({ currentUser }) => {
  const links = [
    ...
    { label: 'My Orders', href: '/orders', isSignedIn: true },
    { label: 'Sell Tickets', href: '/tickets/new', isSignedIn: true },
    { label: 'Sign Out', href: '/auth/sign-out', isSignedIn: true },
  ];

  const linksToShow = currentUser
    ? links.filter((link) => link.isSignedIn)
    : links.filter((link) => !link.isSignedIn);
